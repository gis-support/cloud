...
class TestSettings(BaseTest):
    ...
    def test_distance_get_geojson(self, client):
        token = self.get_token(client, admin=True)
        path = os.path.join(TEST_DATA_DIR, 'layers')
        # Parki narodowe
        pn_file_request = {
        ...
        data = {
          ...
          "geometry": {
            "type": "Point",
            "coordinates": [ 18.052179169925576, 52.189111014341002 ]
          }
        }
        r = client.post(
            f'/api/analysis/distance/xlsx?token={token}', data=json.dumps(data))
        assert r.status_code == 200
